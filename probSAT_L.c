...
static inline void pickAndFlip() {
  ...
  // int rClause = falseClause[rand() % numFalse];
  ...
  i = 0;
  // print line: c_i H(c_i) p(l_1) ... p(l_k) a
  fprintf(stderr, "L%d %f ", rClause, clauseEntropy[rClause]);
  for(int i = 1; i <= numVars; i++){
    fprintf(stderr, "%d", atom[i]);
  }
  while((var = abs(clause[rClause][i]))){
    fprintf(stderr, " %f", probsBreak[breaks[var]]);
    i++;
  }
  fprintf(stderr, "\n");
